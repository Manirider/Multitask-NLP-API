...
from src.modeling.datasets import SentimentDataset, NERDataset, QADataset
...
def evaluate_ner(model, dataloader, device, label_map):
    ...
    if not id2label:
        labels_list = ["O", "B-PER", "I-PER", "B-ORG",
            ...
            for i in range(labels.shape[0]):
                ...
                for j in range(labels.shape[1]):
                    if labels[i, j] != -100:
                        ...
                        p_labels.append(id2label.get(
                            predictions[i, j].item(), "O"))
...
def evaluate_qa(model, dataloader, device, tokenizer):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            outputs = model(input_ids, attention_mask, task_name="qa",
            ...
            for i in range(input_ids.shape[0]):
                ...
                true_text = tokenizer.decode(
                    true_tokens, skip_special_tokens=True)
...
def train(args):
    try:
        ...
        try:
            ...
            logger.error(
                "Data files not found. Please run src/preprocess.py first.")
            sys.exit(1)

        train_dataloaders = {
            "sentiment": DataLoader(sentiment_ds, batch_size=args.batch_size, shuffle=True),
        ...
        total_steps = sum(len(dl)
                          for dl in train_dataloaders.values()) * args.epochs
        if total_steps == 0:
        ...
        with mlflow.start_run() as run:
            ...
            for epoch in range(args.epochs):
                ...
                iterators = {task: iter(dl)
                             for task, dl in train_dataloaders.items()}
                steps_per_epoch = sum(len(dl)
                                      for dl in train_dataloaders.values())
            ...
            qa_em, qa_f1 = evaluate_qa(
                model, val_dataloaders["qa"], device, tokenizer)
            ...
            export_to_onnx(
                ...
                output_names=["logits", "ner_logits",
            ...
            logger.info(
                "Logged ONNX artifacts (model.onnx + model.quant.onnx).")
